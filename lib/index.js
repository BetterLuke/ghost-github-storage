...
class GhostGithubStorage extends BaseStorage {
  constructor (config) {
    super()
    if (!config || !config.repo || !config.owner || !config.token) {
    ...
    octokit.repos.get({
      ...
      if (error.code === 404) {
        ...
        octokit.repos.create({
          name: this.repo,
          description: 'github repo for ghost assets',
    ...
    return this.getFilename(image)
      ...
      .then(filename => {
        ...
        return octokit.repos.createFile({
          owner: this.owner,
          repo: this.repo,
          path: path.relative(this.localPath, filename),
          message: 'updates',
          content: Buffer.from(bitmap).toString('base64'),
          branch: this.branch
        }).then(() => {
          // check if using github pages
          if (this.repo === 'gh-pages') {
            // only request page build when repo is gh-pages
